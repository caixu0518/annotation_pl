...
unless (-e "3.transcript.ok") {
    ...
    $cmdString = "ParaFly -c command.sam2transfrag.list -CPU $cpu";
    print STDERR (localtime) . ": CMD: $cmdString\n";
    system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
    ...
    unless (-e "TransDecoder.ok") {
        $cmdString = "$TransDecoder_LongOrfs  -m 100 -G universal -t transfrag.strand.fasta -S";
        print STDERR (localtime) . ": CMD: $cmdString\n";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
        $cmdString = "$TransDecoder_Predict  --retain_long_orfs 900  -t transfrag.strand.fasta";
        print STDERR (localtime) . ": CMD: $cmdString\n";
        system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
        ...
        unless ($strand_specific) {
            $cmdString = "$TransDecoder_LongOrfs  -m 100 -G universal -t transfrag.noStrand.fasta";
            print STDERR (localtime) . ": CMD: $cmdString\n";
            system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
            $cmdString = "$TransDecoder_Predict  --retain_long_orfs 900 -t transfrag.noStrand.fasta --train transfrag.strand.fasta.transdecoder_dir/longest_orfs.cds.top_500_longest";
            print STDERR (localtime) . ": CMD: $cmdString\n";
            system("$cmdString") == 0 or die "failed to execute: $cmdString\n";
